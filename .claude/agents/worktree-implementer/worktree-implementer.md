...
This agent supports two plan formats:
...
When invoked, you will receive a task description from the parent conversation. Parse it as follows:
...
1. **Determine the feature name** from the task description
...
3. Read each `subtask.md` that's in scope (all, or the specific ones from the task description)
...
2. **Use Chrome DevTools to validate** (if available)
