...
class TestPostAgentQualityGate(unittest.TestCase):
    def setUp(self) -> None:
        self.hook = import_hook()

    def _mock_git_and_checks(
        self,
        mock_run: MagicMock,
    ...
    def test_all_pass(
        self, mock_exists: MagicMock, mock_stdout: StringIO, mock_run: MagicMock
    ) -> None:
        # Arrange
    ...
    def test_only_backend_changed(
        ...
        frontend_calls = [
            call for call in mock_run.call_args_list if "npm" in str(call)
        ]
    ...
    @patch("sys.stdout", new_callable=StringIO)
    @patch("os.path.exists")
    def test_only_frontend_changed(
        self, mock_exists: MagicMock, mock_stdout: StringIO, mock_run: MagicMock
    ) -> None:
        # Arrange
        mock_exists.return_value = True
    ...
    @patch("sys.stdout", new_callable=StringIO)
    @patch("os.path.exists")
    def test_no_changes_skips_all_checks(
        self, mock_exists: MagicMock, mock_stdout: StringIO, mock_run: MagicMock
    ) -> None:
        # Arrange
        mock_exists.return_value = True
