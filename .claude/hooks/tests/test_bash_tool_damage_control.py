...
class TestBashToolDamageControl(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.validator_path = Path(__file__).parent.parent / "bash-tool-damage-control.py"
    ...
    def assert_blocked(self, command: str, reason_substring: str = ""):
        """Assert that a command is blocked (exit code 2)."""
        exit_code, stdout, stderr = self.validate_command(command)
        self.assertEqual(exit_code, 2, f"Expected command to be blocked: {command}\nstderr: {stderr}")
        if reason_substring:
    ...
    def assert_allowed(self, command: str):
        ...
        exit_code, stdout, stderr = self.validate_command(command)
        self.assertEqual(exit_code, 0, f"Expected command to be allowed: {command}\nstderr: {stderr}")

    def assert_asks(self, command: str, reason_substring: str = ""):
        """Assert that a command requires confirmation (exit code 0 with ask decision)."""
        exit_code, stdout, stderr = self.validate_command(command)
        self.assertEqual(exit_code, 0, f"Expected command to ask for confirmation: {command}")
        try:
            response = json.loads(stdout)
            self.assertEqual(response.get("hookSpecificOutput", {}).get("permissionDecision"), "ask")
            if reason_substring:
    ...
    def test_blocks_rm_rf_current(self):
    ...
    def test_blocks_aws_s3_rm_recursive(self):
        """Block AWS S3 recursive delete."""
        self.assert_blocked("aws s3 rm s3://bucket --recursive", "aws s3 rm --recursive")

    def test_blocks_aws_ec2_terminate(self):
        """Block AWS EC2 instance termination."""
        self.assert_blocked("aws ec2 terminate-instances --instance-ids i-1234", "aws ec2 terminate-instances")

    def test_blocks_gcloud_projects_delete(self):
        """Block GCP project deletion."""
        self.assert_blocked("gcloud projects delete my-project", "gcloud projects delete")

    def test_blocks_terraform_destroy(self):
        """Block terraform destroy."""
        self.assert_blocked("terraform destroy", "terraform destroy")

    def test_blocks_docker_system_prune_all(self):
        """Block docker system prune -a."""
        self.assert_blocked("docker system prune -a", "docker system prune -a")

    def test_blocks_kubectl_delete_all(self):
    ...
    def test_blocks_redis_flushall(self):
        """Block Redis FLUSHALL."""
        self.assert_blocked("redis-cli FLUSHALL", "redis-cli FLUSHALL")

    def test_blocks_mongo_drop_database(self):
        """Block MongoDB dropDatabase."""
        self.assert_blocked("mongosh --eval 'db.dropDatabase()'", "MongoDB dropDatabase")

    def test_blocks_postgres_dropdb(self):
    ...
    def test_asks_delete_with_where(self):
        """Ask confirmation for DELETE with specific ID."""
        self.assert_asks("DELETE FROM users WHERE id = 123", "SQL DELETE with specific ID")
    ...
    def test_blocks_cat_backend_env(self):
    ...
    def test_blocks_delete_claude_md(self):
        """Block deleting CLAUDE.md."""
        self.assert_blocked("rm CLAUDE.md", "no-delete path")

    def test_blocks_delete_readme(self):
    ...
    def test_blocks_kill_all_processes(self):
        """Block kill -9 -1."""
