...
def process_single_newsletter(newsletter, supabase, update=False, quiet=False):
    ...
    sanitized_html = sanitize_content(original_html, "html", PRIVACY_PATTERNS_DICT)
    html_changed = show_diff(original_html, sanitized_html, "HTML", quiet=quiet)

    # Sanitize Text
    original_text = newsletter.get("plain_text") or ""
    sanitized_text = sanitize_content(original_text, "text", PRIVACY_PATTERNS_DICT)
    ...
    if modified and update:
        if not quiet:
            print("\n  Updating database...")
        update_data = {"raw_html": sanitized_html, "plain_text": sanitized_text}
        result = (
            supabase.table("newsletters")
            .update(update_data)
            .eq("id", newsletter["id"])
...
def main():
    ...
    group.add_argument(
        "newsletter_id", nargs="?", help="UUID of a specific newsletter to process"
    )
    group.add_argument(
        "--all", action="store_true", help="Process all newsletters in the database"
    )

    parser.add_argument(
        "--update", action="store_true", help="Commit changes to database"
    )
    parser.add_argument(
        "--quiet", action="store_true", help="Minimal output (best for bulk updates)"
    )
    ...
    if args.all:
        ...
        for i, newsletter in enumerate(newsletters):
            ...
            modified = process_single_newsletter(
